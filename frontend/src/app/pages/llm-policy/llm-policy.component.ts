...
export class LLMPolicyComponent implements OnInit {
  ...
  themeIcon(): string {
      return this.themeMode === 'dark' ? 'eye-invisible' : 'bulb';
  }
  ...
  runDueModelMaintenance(): void {
    ...
    this.llmPolicyService.runDueModelMaintenance().subscribe({
      next: (run) => {
        ...
        this.loadModelMaintenanceHistory();
        const summary = `${run.checked}/${run.eligible} due checks, ${run.reused} still current, ${run.updated} updated, ${run.failed} failed`;
        if (run.failed > 0) {
      ...
      error: () => {
        this.maintaining = false;
        this.notification.error('Model maintenance failed', 'HAI could not complete the due model checks. Routing will retain its existing safety gates.');
      },
    });
  }
  ...
  maintenanceColor(record: ILLMModelMaintenanceResult): string {
    if (record.status === 'updated' || record.status === 'installed') return 'green';
    if (record.status === 'current' || record.status === 'provider_managed') return 'blue';
    if (record.status === 'not_enforced') return 'default';
    return 'red';
  }
  ...
  maintenanceScheduleText(record: ILLMModelMaintenanceResult): string {
    ...
    if (Number.isNaN(due.getTime())) {
      return 'Next check time is unavailable.';
    }
    return `Next check due ${due.toLocaleString()}.`;
  }
