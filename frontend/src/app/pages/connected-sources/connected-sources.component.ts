...
type SourceAction = 'connect' | 'gmail' | 'odoo' | 'import' | 'folder' | 'whatsapp' | 'search' | 'graph';
...
export class ConnectedSourcesComponent implements OnInit {
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.notification.error(
            ...
            error?.error?.error || 'Check the local Docling runner, selected document folder, and any pre-provisioned PDF artifacts.'
          );
        },
      });
  }
  ...
  sourceActionLabel(source: IConnectedSource): string {
    if (source.connectorKey === 'whisper-audio') return 'Transcribe';
    if (source.connectorKey === 'docling-documents') return 'Extract documents';
    return 'Sync';
  }

  sourceActionTitle(source: IConnectedSource): string {
    if (source.connectorKey === 'whisper-audio') {
      return 'Transcribe the source\'s explicit local audio folder. HAI never records a microphone or accepts uploaded audio.';
    }
    if (source.connectorKey === 'docling-documents') {
      return 'Extract text from the source\'s explicit local document folder. HAI never accepts uploads, chooses models, or enables OCR, table extraction, or cloud services.';
    }
  ...
  loadKnowledgeGraph(): void {
    ...
    this.sourceService
      .knowledgeGraph(this.searchForm.value.projectKey, this.includeArchived, this.graphIncludeSensitive)
      .pipe(timeout(this.loadTimeoutMs), finalize(() => (this.graphLoading = false)))
      .subscribe({
        next: (graph) => {
          this.knowledgeGraph = graph;
          this.updateSourceActions();
        },
        error: () => this.notification.error('Knowledge view unavailable', 'The source-linked candidate view could not load.'),
      });
  }

  knowledgeGraphSourceLabel(refs: IKnowledgeGraphSourceRef[]): string {
    const ref = refs && refs.length ? refs[0] : undefined;
