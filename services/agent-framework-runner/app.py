"""Internal-only Microsoft Agent Framework review-only planning runner."""
...
def configured() -> tuple[str, str, str]:
    base_url = os.environ.get("HAI_AGENT_FRAMEWORK_LOCAL_MODEL_BASE_URL", "").strip().rstrip("/")
    model_id = os.environ.get("HAI_AGENT_FRAMEWORK_LOCAL_MODEL_ID", "").strip()
    api_key = os.environ.get("HAI_AGENT_FRAMEWORK_LOCAL_MODEL_API_KEY", "").strip() or "local-no-key"
...
def request_digest(request: str, criteria: list[str]) -> str:
    ...
    # Go's encoding/json (used by the owner-side bridge) escapes these HTML
    # characters. Keep both sides byte-for-byte compatible before hashing.
...
def validate_proposal(value: object) -> dict:
    expected = {"goal", "successCriteria", "nextSteps", "risk", "requiresApproval", "reasons", "uncertainties"}
...
async def create_proposal(base_url: str, model_id: str, api_key: str, request: str, criteria: list[str]) -> dict:
    ...
    client = OpenAIChatCompletionClient(model=model_id, base_url=base_url, api_key=api_key)
    ...
    try:
        ...
        final = await reviewer.run(review_prompt)
        return json.loads(final.text.strip())
    finally:
...
def probe() -> dict:
    ...
    return {"status": "ok", "engine": engine_name(), "modelId": model_id, "modelEndpoint": base_url}


class Handler(BaseHTTPRequestHandler):
    server_version = "HAI-Agent-Framework/1.0"
    ...
    def do_GET(self):
        ...
        try:
            ...
            self.respond(200, {"status": "ok", "configured": True, "engine": engine_name(), "modelId": model_id, "modelEndpoint": base_url, "scope": "configured local planning runner only"})
        except RequestError:
            self.respond(200, {"status": "ok", "configured": False, "engine": engine_name(), "scope": "runner is healthy; local model is not configured"})
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.respond(502, {"error": "local Agent Framework runner could not produce a valid bounded proposal"})
