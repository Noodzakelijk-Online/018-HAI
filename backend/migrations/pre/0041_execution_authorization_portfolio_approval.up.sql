ALTER TABLE public.pursuit_portfolio_execution_proposal_decisions
    ADD CONSTRAINT uq_pursuit_portfolio_execution_decision_owner_id
    UNIQUE (owner_identity, id);

ALTER TABLE public.execution_authorization_receipts
    ADD COLUMN portfolio_proposal_decision_id uuid;
...
ALTER TABLE public.execution_authorization_receipts
    ADD CONSTRAINT fk_execution_authorization_receipt_portfolio_decision
        FOREIGN KEY (owner_identity, portfolio_proposal_decision_id)
        REFERENCES public.pursuit_portfolio_execution_proposal_decisions
            (owner_identity, id)
...
CREATE INDEX idx_execution_authorization_receipts_portfolio_decision
    ON public.execution_authorization_receipts
    (owner_identity, portfolio_proposal_decision_id, evaluated_at DESC)
