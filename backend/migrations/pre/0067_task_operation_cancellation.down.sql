DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM public.task_operations WHERE status = 'canceled' LIMIT 1) THEN
        RAISE EXCEPTION 'refusing to remove task operation cancellation semantics while canceled audit state exists';
    END IF;
END;
$$;
...
ALTER TABLE public.task_operations
    ADD CONSTRAINT chk_task_operations_status
        CHECK (status IN ('running', 'completed', 'needs_review')),
