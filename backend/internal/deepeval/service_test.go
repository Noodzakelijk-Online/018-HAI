...
func TestDeepEvalBridgeUsesOnlyConfiguredFixedSuite(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/healthz":
			...
			_, _ = w.Write([]byte(`{"status":"ok","engine":"deepeval 4.1.1","configured":true,"suite":"hai_source_grounding_regression_v1","metric":"FaithfulnessMetric","modelId":"qwen2.5:7b"}`))
		case "/v1/run":
