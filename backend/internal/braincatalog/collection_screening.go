...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"AI Agent Frameworks":                  {CollectionCandidate, []string{"continue", "openhands", "crewai", "autogen", "pydantic-ai"}, "Only reviewed, narrow adapters may use these orchestration patterns; HAI retains policy, audit, and final execution authority."},
	...
	"Data Integration":                     {CollectionCandidate, []string{"airbyte", "cloudquery"}, "A read-first connector bridge is a possible future adapter, but credentials, scopes, cursors, retention, and deletion must be reviewed first."},
	...
	"RAG Frameworks":                       {CollectionCandidate, []string{"langchain", "llamaindex", "mem0", "letta", "langmem", "haystack", "anythingllm"}, "AnythingLLM is a review-first local workspace/RAG option; HAI keeps one source-linked memory plane and does not create a second memory authority."},
	"LLM Inference Engines":                {CollectionCandidate, []string{"ollama", "llama-cpp", "vllm", "localai"}, "Local inference is already routed through HAI's provider policy and live-probe gates; alternate local servers remain review-first options for a measured serving gap and model servers remain operator-installed."},
	...
	"AI Coding Assistants":                 {CollectionCandidate, []string{"tabby", "continue", "openspec"}, "Coding assistance requires a repository-specific, review-first adapter; no assistant receives implicit source or write access."},
	"AI Browser Agents":                    {CollectionCandidate, []string{"browser-use", "playwright", "omniparser"}, "Browser autonomy is high-risk. HAI uses an allowlisted, approval-gated verification path and treats agentic browsing as a separate adapter review."},
	...
	"Multimodal AI":                        {CollectionCandidate, []string{"whisper-cpp", "pipecat"}, "Local speech transcription or multimodal intake can enrich approved work, but audio capture, retention, and source attribution must be reviewed first."},
	"AI Evaluation & Testing":              {CollectionCandidate, []string{"promptfoo", "langfuse", "garak", "deepeval", "lm-eval-harness", "openllmetry", "opik", "openai-evals"}, "Evaluation tools are candidates for controlled regression and safety checks; test prompts and provider credentials remain sensitive, while OpenAI Evals remains held for licence review."},
	...
	"AI Observability":                     {CollectionCandidate, []string{"langfuse", "openllmetry", "openlit", "phoenix", "opik"}, "Trace and evaluation observability is a strong candidate, subject to local hosting, redaction, retention, egress, and licence review."},
