...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"MCP Servers":                          {CollectionCandidate, []string{"mcp-inspector", "fastmcp", "github-mcp-server", "playwright-mcp", "google-genai-toolbox"}, "MCP server discovery is limited to preflight and allowlist review; local tool servers, GitHub, browser, and database bridges remain review-first options, and a catalog record never launches a server, creates credentials, or calls a tool."},
	...
	"LLM Inference Engines":                {CollectionCandidate, []string{"ollama", "llama-cpp", "vllm"}, "Local inference is already routed through HAI's provider policy and live-probe gates; vLLM is a review-first option for a measured serving gap and model servers remain operator-installed."},
	...
	"AI Safety & Alignment":                {CollectionCandidate, []string{"nemo-guardrails", "guardrails-ai", "presidio", "garak"}, "Safety tools may strengthen validation and redaction, but require a data-handling and false-positive review before they influence actions."},
	...
	"AI Evaluation & Testing":              {CollectionCandidate, []string{"promptfoo", "langfuse", "garak", "deepeval", "lm-eval-harness", "openllmetry"}, "Evaluation tools are candidates for controlled regression and safety checks; test prompts and provider credentials remain sensitive."},
	...
	"AI Observability":                     {CollectionCandidate, []string{"langfuse", "openllmetry", "openlit", "phoenix"}, "Trace and evaluation observability is a strong candidate, subject to local hosting, redaction, retention, egress, and licence review."},
	...
	"AI Red Teaming":                       {CollectionCandidate, []string{"promptfoo", "garak", "pyrit"}, "Safety testing is valuable but must use redacted fixtures, provider restrictions, and a no-write evaluation boundary; archived PyRIT remains excluded."},
	...
	"Agent Harness":                        {CollectionCandidate, []string{"autogen", "crewai", "letta", "swe-agent", "taskweaver"}, "Harness patterns are candidates for HAI-native adapters only; no external agent may self-authorize tools, providers, or side effects, and archived upstreams remain excluded."},
}
