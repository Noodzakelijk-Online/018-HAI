...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"Business Management":                  {CollectionCandidate, []string{"odoo"}, "HAI has an opt-in, read-only Odoo JSON-2 source adapter with fixed model and field allowlists. It does not import or replace an ERP, write back, or authorize financial/customer actions."},
