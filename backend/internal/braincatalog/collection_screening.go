...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"AI Observability":                     {CollectionCandidate, []string{"langfuse", "openllmetry", "openlit", "phoenix", "opik", "evidently", "whylogs"}, "Trace and evaluation observability is a strong candidate, subject to local hosting, redaction, retention, egress, and licence review. Whylogs is held as a stale-release profiling reference rather than a new telemetry authority."},
	...
	"Agent Sandboxing":                     {CollectionReference, []string{"e2b", "daytona", "taskweaver"}, "E2B remains an external-service reference only. Daytona is excluded because its public upstream is unmaintained and core moved private; archived TaskWeaver is also excluded. No collection item authorizes a broad execution environment."},
