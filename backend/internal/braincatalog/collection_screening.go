...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"MCP Servers":                          {CollectionCandidate, []string{"mcp-inspector", "fastmcp"}, "MCP server discovery is limited to preflight and allowlist review; FastMCP is a review-first option for a narrowly owned local service, and a catalog record never launches a server or calls a tool."},
