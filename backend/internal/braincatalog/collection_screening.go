...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"MCP Servers":                          {CollectionCandidate, []string{"mcp-inspector", "playwright-mcp", "fastmcp"}, "MCP server discovery is limited to preflight and allowlist review; FastMCP is a review-first option for a narrowly owned local service, and a catalog record never launches a server or calls a tool."},
	...
	"RAG Frameworks":                       {CollectionReference, []string{"langchain", "llamaindex", "mem0", "letta", "haystack"}, "HAI keeps one source-linked memory plane; framework patterns can inform a reviewed adapter but do not create a second memory authority."},
	...
	"AI Safety & Alignment":                {CollectionCandidate, []string{"nemo-guardrails", "guardrails-ai", "presidio", "llm-guard", "garak"}, "Safety tools may strengthen validation and redaction, but require a data-handling and false-positive review before they influence actions."},
	...
	"AI Observability":                     {CollectionCandidate, []string{"langfuse", "openllmetry"}, "Trace and evaluation observability is a strong candidate, subject to local hosting, redaction, retention, and egress review."},
