...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"LLM Inference Engines":                {CollectionRepresented, []string{"ollama", "llama-cpp", "vllm", "localai"}, "HAI has loopback-only provider profiles for Ollama, llama.cpp, LocalAI, and vLLM. Every model server remains operator-installed, explicitly configured, live-probed, and governed by the local-first policy."},
