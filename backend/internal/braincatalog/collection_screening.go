...
var ossInsightCollectionDecisions = map[string]collectionDecision{
	...
	"Coding Agents":                        {CollectionCandidate, []string{"opencode", "cline", "aider", "openhands", "serena", "goose"}, "Write-capable coding agents remain workspace-confined, review-first candidates with explicit model, tool, and network boundaries. Serena is limited to a read-only semantic-context review; Goose remains a reference because it would otherwise create a competing general-purpose agent control plane."},
	...
	"LLM Inference Engines":                {CollectionRepresented, []string{"ollama", "llama-cpp", "vllm", "localai", "mistral-rs"}, "HAI has loopback-only provider profiles for Ollama, llama.cpp, LocalAI, and vLLM; mistral.rs is a separate review-first OpenAI-compatible local-server candidate. Every model server remains operator-installed, explicitly configured, live-probed, and governed by the local-first policy."},
