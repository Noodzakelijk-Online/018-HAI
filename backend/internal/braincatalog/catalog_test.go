...
func TestEntriesAreTransparentAndDisabledByPolicy(t *testing.T) {
	...
	for _, id := range []string{"openllmetry", "deepeval", "airbyte", "browser-use", "nemo-guardrails", "garak", "tabby", "github-mcp-server", "playwright-mcp", "google-genai-toolbox", "qodo-pr-agent", "swe-agent", "openlit", "opik", "deepteam", "openspec", "pipecat", "livekit-agents"} {
	...
	for _, id := range []string{"presidio", "guardrails-ai", "lm-eval-harness", "promptfoo", "evidently", "ragflow", "anythingllm", "serena", "odoo", "cloudquery"} {
