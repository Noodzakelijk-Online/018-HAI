...
var integratedImplementationBoundaries = map[string]ImplementationBoundary{
	...
	"chatgpt-codex-mcp-daemon":         {"conversation-history task context", "/api/v1/task", "backend/internal/chatgptlogs/service.go", "opt-in local search tool only; bounded untrusted context and no process launch"},
