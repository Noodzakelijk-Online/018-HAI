...
func (h *Handler) Run(c *gin.Context) {
	...
	if c.Request.ContentLength != 0 {
		if err := c.ShouldBindJSON(&request); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "workflowId must be a valid JSON field when supplied"})
			return
		}
	}
	run, err := h.service.Run(c.Request.Context(), owner(c), c.Param("id"), request.WorkflowID)
