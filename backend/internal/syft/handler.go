...
func (h *Handler) Probe(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadGateway, gin.H{"error": "local Syft runner could not be verified"})
		return
	}
...
func (h *Handler) Inventory(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&request); err != nil || strings.TrimSpace(request.WorkspaceID) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "workspaceId is required; HAI never accepts a filesystem path, source content, SBOM export, command, scanner configuration, or report destination"})
		return
	}
	...
	case err != nil:
		c.JSON(http.StatusBadGateway, gin.H{"error": "local Syft runner could not return a redacted aggregate inventory result"})
	default:
		c.JSON(http.StatusOK, result)
	}
}
