...
func (s *fakeService) Probe(context.Context) (*ProbeResult, error) {
	return &ProbeResult{Reachable: true, Engine: "syft 1.48.0"}, nil
}
func (s *fakeService) Inventory(_ context.Context, workspaceID string) (*InventoryResult, error) {
	s.workspace = workspaceID
	if workspaceID != "review-snapshot" {
...
func TestInventoryHandlerRequiresNamedWorkspace(t *testing.T) {
	...
	if response.Code != http.StatusBadRequest || service.workspace != "" {
		t.Fatalf("response=%d workspace=%q body=%s", response.Code, service.workspace, response.Body.String())
	}
}
...
func TestInventoryHandlerPassesOnlyWorkspaceIdentifier(t *testing.T) {
	...
	context.Request.Header.Set("Content-Type", "application/json")
	handler.Inventory(context)
	if response.Code != http.StatusOK || service.workspace != "review-snapshot" {
		t.Fatalf("response=%d workspace=%q body=%s", response.Code, service.workspace, response.Body.String())
	}
}
