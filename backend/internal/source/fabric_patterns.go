...
// The Fabric patterns adapter imports only manually installed prompt-pattern
// files. Patterns are useful review material, but they are untrusted input:
// no pattern is executed, automatically attached to a model, or allowed to
// change HAI policy, approval, routing, runtime, tool, or workspace controls.
...
// ManualPlanningContextOnlyConnectorKeys returns connector keys whose content
// may be inspected in HAI but must not be loaded into a normal task plan. The
// caller needs an explicit, reviewed attachment path before any such text can
// become labelled untrusted planning context.
