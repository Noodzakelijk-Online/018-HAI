...
func (s handlerStubService) Probe(context.Context) (*ProbeResult, error) {
	return &ProbeResult{Reachable: true, Engine: "trivy 0.72.0", CheckedAt: time.Now()}, nil
}
...
func TestHandlerRequiresAuthenticatedSubjectAndReturnsAggregate(t *testing.T) {
	...
	result := &ScanResult{Status: "completed", Engine: "trivy 0.72.0", FindingCount: 0, Severities: []SeverityCount{}, DurationMS: 1, ResultDigest: strings.Repeat("a", 64)}
	...
	if response.Code != http.StatusOK || strings.Contains(response.Body.String(), "vulnerability") || !strings.Contains(response.Body.String(), "findingCount") {
