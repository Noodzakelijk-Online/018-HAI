// Package trivy exposes a deliberately narrow configuration-security boundary.
// It scans named, operator-allowlisted read-only snapshots using Trivy's
// offline misconfiguration scanner. It never exposes source, file paths,
// rules, findings, raw reports, or remediation steps.
package trivy
...
const (
	enabledEnv       = "HAI_TRIVY_ENABLED"
	runnerURLEnv     = "HAI_TRIVY_RUNNER_URL"
	tokenEnv         = "HAI_TRIVY_RUNNER_TOKEN"
	workspacesEnv    = "HAI_TRIVY_WORKSPACES"
	timeoutEnv       = "HAI_TRIVY_TIMEOUT_SECONDS"
...
var (
	ErrNotConfigured = errors.New("local Trivy runner is not configured")
	ErrUnavailable   = errors.New("local Trivy runner is unavailable")
	ErrWorkspace     = errors.New("workspace is not approved for configuration security scanning")
	workspacePattern = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$`)
	severityPattern  = regexp.MustCompile(`^(critical|high|medium|low|unknown)$`)
)
...
func (s *service) Status() Status {
	...
	status := Status{
		Enabled: s.enabled, Configured: s.configured(), Provider: "Trivy local aggregate configuration security scanner", Workspaces: workspaces, ConfigError: s.configErr,
		Capabilities: []string{"operator-triggered aggregate offline misconfiguration analysis for an explicitly allowlisted local configuration snapshot", "redacted failed-check total, severity counts, duration, and result digest"},
		Restrictions: []string{"no caller-selected path, source content, container image, Git repository, cloud account, Kubernetes cluster, package, secret, vulnerability, rule, finding, file path, raw report, cloud upload, external network, write operation, remediation, or scheduled scan", "the runner uses only Trivy's offline configuration scanner with policy and database updates disabled", "a scan result cannot approve an action, verify task completion, alter source, or automatically block a workflow"},
		Scope:        "Owner-triggered local configuration-security evidence only. HAI returns aggregate redacted metadata from a configured snapshot; results require human review in the original workspace and do not become source facts or execution decisions.",
	}
	if s.runnerURL != nil {
...
func (s *service) Probe(ctx context.Context) (*ProbeResult, error) {
	...
	request.Header.Set("User-Agent", "HAI-Trivy-Configuration-Review/1.0")
...
func (s *service) Scan(ctx context.Context, workspaceID string) (*ScanResult, error) {
	...
	request.Header.Set("User-Agent", "HAI-Trivy-Configuration-Review/1.0")
	request.Header.Set("X-HAI-Trivy-Token", s.token)
	response, err := s.client.Do(request)
	if err != nil {
		return nil, ErrUnavailable
	}
...
func parseRunnerURL(raw string) (*url.URL, string) {
	...
	if host != "localhost" && host != "host.docker.internal" && host != "trivy-runner" && net.ParseIP(host) == nil {
		return nil, runnerURLEnv + " must resolve to localhost, host.docker.internal, trivy-runner, or a literal local IP"
	}
...
func validEngine(value string) bool {
	return strings.HasPrefix(strings.TrimSpace(value), "trivy ") && len(value) <= 160
}

func validResult(result ScanResult, workspaceID string) bool {
	if result.Status != "completed" || !validEngine(result.Engine) || result.WorkspaceID != workspaceID || result.FindingCount < 0 || result.FindingCount > 100000 || len(result.Severities) > 5 || result.DurationMS < 0 || result.DurationMS > int64((5*time.Minute)/time.Millisecond) {
		return false
	}
