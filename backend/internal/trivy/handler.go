...
func (h *Handler) Probe(c *gin.Context) {
	...
	case err != nil:
		c.JSON(http.StatusBadGateway, gin.H{"error": "local Trivy runner could not be verified"})
	default:
...
func (h *Handler) Scan(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&request); err != nil || strings.TrimSpace(request.WorkspaceID) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "workspaceId is required; HAI never accepts a filesystem path, source content, container image, Git repository, cloud account, Kubernetes cluster, secret, vulnerability, finding, rule, report, command, scanner configuration, or remediation request"})
		return
	}
	...
	case err != nil:
		c.JSON(http.StatusBadGateway, gin.H{"error": "local Trivy runner could not return a redacted aggregate configuration security result"})
	default:
		c.JSON(http.StatusOK, result)
	}
}
