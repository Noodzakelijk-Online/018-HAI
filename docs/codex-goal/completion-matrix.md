...
| 032 | Docker & deployment readiness | Implemented locally | One canonical source-built topology lives in `docker-compose.local.yml`; the root, backend, IDP, gate, and Kafka Compose entrypoints delegate to it, and the development override changes behavior without adding infrastructure. CI validates every entrypoint. On 2026-08-09 the standard root entrypoint resolved runtime-equivalent configuration, ran the ten steady-state services healthy on Docker Desktop, completed the one-shot migrator, and returned HTTP 200 from `/readyz`. A fresh-clone run on each release target remains an operator release gate, not an implementation gap. |
