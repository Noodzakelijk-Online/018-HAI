...
| Promptfoo | LLM DevTools | Candidate, local bridge implemented | Fixed synthetic local safety regression | The opt-in `safety-evaluation` runner uses one reviewed local model endpoint and a shipped six-case suite. It returns aggregate metadata only; no provider, prompt, source, test data, routing, policy, or action is caller-controlled. |
