...
| BH-6 | Medium (supply chain) | `backend` deps | `govulncheck` found 20 code-affecting vulnerabilities (`golang.org/x/net`, `github.com/jackc/pgx/v5`, Go stdlib). | Open (advisory in CI) — exact remediation + why not yet a blocking gate in `docs/dependency-vulnerabilities.md`; a coordinated dependency+toolchain+vet upgrade is required. |
