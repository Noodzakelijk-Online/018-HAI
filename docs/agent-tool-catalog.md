...
| [A2A Protocol](https://github.com/a2aproject/A2A) | Integrated, opt-in | Local authenticated planning interoperability | HAI exposes a disabled-by-default Agent Card and a narrow `tasks/send` JSON-RPC subset for one named local bearer-token peer. It maps to a side-effect-free HAI planning preview and returns only a bounded proposal; no task/workflow persistence, source refresh, approval, execution, peer discovery, streaming, file/data input, memory/source disclosure, or tool invocation is available. |
