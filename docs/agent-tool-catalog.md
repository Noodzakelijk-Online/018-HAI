...
| [Presidio](https://github.com/data-privacy-stack/presidio) | Candidate | Local PII detection and redaction second-pass | The maintained project moved from the Microsoft GitHub namespace. HAI retains its deterministic privacy controls and can only add Presidio through a local, review-first false-positive and retention boundary; no automated detector can establish that sensitive data is absent. |
