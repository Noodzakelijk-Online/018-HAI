...
| [Promptfoo](https://github.com/promptfoo/promptfoo) | Candidate, local bridge implemented | Fixed synthetic local safety regression | The optional `safety-evaluation` profile runs a shipped six-case local prompt-injection and high-risk-action suite against one configured OpenAI-compatible endpoint. Its health probe requires that model and suite configuration, the runner clears inherited proxy variables and runs unprivileged, and it returns aggregate pass/fail metadata only. It cannot accept real prompts, sources, models, providers, endpoints, commands, or alter HAI decisions. |
