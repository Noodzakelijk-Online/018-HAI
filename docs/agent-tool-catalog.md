...
| Worker Control | Integrated, opt-in | Read-only commitment and reminder source | One operator-owned Worker Control endpoint and revocable `vwc_hai_` key only. HAI accepts a bounded owner-scoped event feed with constrained provenance, marks every import sensitive, and has no Worker Control write path. |
