...
| Health/readiness vs the **actual** stack | ✅ | ✅ | 🟡 | 🟡 | `/readyz` distinguishes ready/degraded/not_ready (4 tests). DB layer **proven against real Postgres 17** (see §3 migrations). Full live Redis/Kafka probe run: deferred. |
...
| Durable worker (scheduling/retry) | ✅ | ✅ | ✅ | ⬜ | **Built** (`internal/durablejob`): persisted jobs, `RunAt` scheduling, bounded retry with backoff, dead-lettering, lease-based crash recovery, panic containment. 8 tests pass incl. **real Postgres**: survives process restart, 25 jobs × 2 concurrent workers each executed exactly once (`FOR UPDATE SKIP LOCKED`), orphaned leases reclaimed. **Now wired in:** source scheduling runs on it (`internal/source/durable_scheduler.go`) — a singleton `source.scan` job enqueues one retryable `source.sync` job per due source, so a failing source retries on its own backoff instead of stalling the sweep; it falls back to the legacy ticker (with a log line) if the queue is unreachable. 5 further tests cover this. **Still in-process:** the workflow and ambient schedulers. |
