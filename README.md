...
| LLM routing | Local-first routing, seven-tier model policy, named configurable provider integrations only, local/OpenAI-compatible endpoint probes, fallback logging, cached/repeated-prompt controls, request-cancellable direct generation, and a EUR 0 paid default. Synthetic tier-filler providers are rejected by regression tests. | A configured endpoint is not live-proven until it passes a bounded probe and validated task. Paid generation remains disabled by default. Canceled direct API calls are recorded as stopped with zero local token/cost accounting; task-worker context propagation and remote-provider disconnect behavior remain separate acceptance gates. |
