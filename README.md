...
| External agent catalog | Candidate capabilities from the E2B awesome-list are curated into task planning and a read-only API. Continue, OpenHands, CrewAI, and Aider are candidates; E2B is reference-only; AutoGPT requires licensing review; AutoGen is a guarded legacy-compatibility profile; MetaGPT is excluded for new integrations. | A catalog entry is not an installed dependency or executable runtime. See [the catalog decision record](docs/agent-tool-catalog.md). |
