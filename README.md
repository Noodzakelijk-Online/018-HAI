...
| Ambient planning | Dashboard-triggered scans, proposals, and scan history are owner-scoped and suggestion-only. The five ambient need settings are shared defaults. | Add authenticated per-owner need profiles and exercise them with two local accounts before treating ambient configuration as multi-user isolated. |
...
- The five default need dimensions are health/capacity, safety/stability, relationships/belonging, reputation/capability, and growth/self-direction. They are shared planning defaults in the current database schema, not per-owner profiles. They are planning preferences, not diagnoses or judgments about a person's worth or status.
