...
| Operations | nginx gateway, IDP, Postgres, Redis, Kafka, health/readiness, support bundle, doctor/reconcile/migrate commands, versioned SQL migrations, a durable job runner (persisted retry + crash recovery), CI, Compose validation, and local smoke coverage. | The durable worker (`internal/durablejob`) is verified against real Postgres, but the existing source schedulers have **not yet been migrated onto it** — those remain in-process, single-node. This is not a distributed or HA worker platform. |
