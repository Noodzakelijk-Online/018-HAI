...
- **Knowledge and memory:** encrypted user-authorized conversation capture, compact context memory, retrieval/search/filter/pagination, deduplication, corrections, export/deletion planning, and source provenance. Authenticated memory APIs, AI-conversation imports, and source-derived consolidation are owner-scoped; background workflow/task/ambient producers still need workflow-ownership propagation before every derived lesson can be scoped automatically.
