...
| Source ingestion | Allowlisted local files; MBOX/EML, ICS, Trello JSON, WhatsApp exports, Odoo/HERP snapshots, normalized JSON feeds, synced document folders, read-only GitHub sync, read-only Gmail (Google OAuth), and read-only Trello REST sync. | Gmail (Google OAuth) and Trello have live read-only API adapters that are implemented but unconfigured by default and not yet live-proven. Calendar, Drive, WhatsApp, and browser accounts remain export/local-folder paths, not live connectors. |
...
| Operations | nginx gateway, IDP, Postgres, Redis, Kafka, health/readiness, support bundle, doctor/reconcile/migrate commands, versioned SQL migrations, a durable job runner (persisted retry + crash recovery), CI, Compose validation, and local smoke coverage. | **Source** scheduling runs on the durable worker: a singleton scan job enqueues one retryable sync job per due source, with backoff and lease-based crash recovery. It falls back to the in-process ticker (and logs that it did) if the queue is unreachable. The **workflow and ambient** schedulers are still in-process tickers. This is a single-node worker, not a distributed or HA platform. |
...
| External accounts | Local/export ingestion and read-only GitHub sync are available. Read-only Gmail (Google OAuth) and Trello REST adapters are implemented but unconfigured by default and not yet live-proven (they need credentials plus a sandbox acceptance run). Live Drive, Calendar, WhatsApp, and browser connectors are not implemented. |
