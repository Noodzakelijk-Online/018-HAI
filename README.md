...
| LLM routing | Local-first routing, seven-tier model policy, named configurable provider integrations only, local/OpenAI-compatible endpoint probes, fallback logging, cached/repeated-prompt controls, and a EUR 0 paid default. Synthetic tier-filler providers are rejected by regression tests. | A configured endpoint is not live-proven until it passes a bounded probe and validated task. Paid generation remains disabled by default. |
...
memory (83.8%). The production Angular build remains lazy-loaded at 652.75 kB
initial raw output and approximately 86.19 kB estimated transfer size.
