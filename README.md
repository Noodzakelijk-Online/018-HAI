...
| External capability catalog | Candidate capabilities from Awesome AI Agents and OSS Insight Collections are curated into task planning and a read-only API. Candidates include local model gateway, Postgres semantic retrieval, durable workflow, metrics, MCP pre-activation, and reviewed agent profiles. | A catalog entry is not an installed dependency or executable runtime. See [the catalog decision record](docs/agent-tool-catalog.md) and [OSS Insight curation](docs/ossinsight-brain-curation.md). |
