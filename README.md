...
- **Knowledge and memory:** encrypted user-authorized conversation capture, compact context memory, retrieval/search/filter/pagination, deduplication, corrections, export/deletion planning, and source provenance. AI-conversation history is scoped to the authenticated owner; the older compact context-memory store is still being migrated to the same boundary.
